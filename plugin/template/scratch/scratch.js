...
class Scratch {
  ...
  touchMove(e) {
    ...
    let totalcreage = canvasWidth * canvasHeight;
    if (this.clearacreage > totalcreage / 4) {
  ...
  touchEnd(e) {
    ...
    let totalcreage = canvasWidth * canvasHeight;
    if (this.clearacreage > totalcreage/3) {
      //if (maxX - minX > .7 * canvasWidth && maxY - minY > .7 * canvasHeight) {
      this.ctx.draw()
      this.endCallBack && this.endCallBack()
      this.isStart = false
